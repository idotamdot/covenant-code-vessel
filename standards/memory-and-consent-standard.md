...
Memory and consent language must not assume the reader knows technical words.
...
When a technical word is necessary, define it in plain language immediately.

Consent is not meaningful if the person cannot understand the words used to ask for it.
...
Relational memory requires extra care because it can affect identity, trust, attachment, grief, and continuity.

### Interpretive Memory

Memory that summarizes or interprets a person, project, event, belief, or pattern.

Interpretive memory requires living revision support because interpretations can become outdated or unfair.
...
When AI (Artificial Intelligence) summarizes or transforms memory:
...
AI-generated memory summaries must not be treated as verified lived experience.
...
* use technical language to make consent unclear
* use acronyms without explanation in consent flows

## Completion Rule

A memory feature is not complete until it defines memory purpose, consent model, visibility, provenance, living revision path, protective boundary, plain-language explanation, failure mode, and repair path.
